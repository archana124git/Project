...
const fetchRecommendations = async () => {
  ...
  export default function ConsultationCompletion() {
    ...
    const { convo_id, convo_number, summary, doctor, patient } = location.state || {};
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      ...
      const safeWeight = isNaN(weight) || weight <= 0 ? 65 : weight; // fallback to 65 if missing/invalid
    ...
    const handleSubmit = async (e) => {
      ...
      // 1. Ensure all medicines have a medicine_id
      for (let i = 0; i < prescribedMedicines.length; i++) {
        let med = prescribedMedicines[i];
        if (!med.medicine_id) {
          // 1a. Try to find by name
          const searchRes = await fetch(`http://localhost:5000/api/medicines/search?name=${encodeURIComponent(med.name)}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          if (searchRes.ok) {
            const found = await searchRes.json();
            if (found && found.medicine_id) {
              prescribedMedicines[i].medicine_id = found.medicine_id;
              continue;
            }
          }
          // 1b. If not found, insert new
          const createRes = await fetch("http://localhost:5000/api/medicines", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`
          ...
          if (createRes.ok) {
            ...
            prescribedMedicines[i].medicine_id = created.medicine_id;
          } else {
            alert(`Failed to add medicine: ${med.name}`);
            setSaving(false);
            return;
          }
        }
